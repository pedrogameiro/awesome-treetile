...
local function do_treesome(p)
    ...
    -- geo_tree is the tree structure to record the all of clients and geometry
    ...
    if trees[tag].n ~= n then
        ...
        if trees[tag].clients then
            ...
            if diff and #diff == 2 then
                ...
                trees[tag].geo_tree:swapLeaves(hash(diff[1]), hash(diff[2]))
    ...
    -- A client is added. Put it is in the tree. 
            ...
            if not trees[tag].t or not trees[tag].t:find(hash(c)) then
                ...
                if trees[tag].t and focus and hash(c) ~= hash(focus)  and not layoutSwitch then
                    ...
                    if prevClient then
                        ...
                        -- ?? is the following setence needed?
...
local function mouse_resize_handler(c, corner, x, y)
    ...
    local parent_node = trees[tag].geo_tree:getParent(hash(c))
    local parent_geo
    ...
    capi.mousegrabber.run(function (_mouse)
                              for k, v in ipairs(_mouse.buttons) do
                                  if v then
                                      ...
                                      local sib_node = trees[tag].geo_tree:getSibling(hash(c))
